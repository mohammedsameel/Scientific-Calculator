...
class Calculator:
    def __init__(self, master):
        ...
        top_frame = tk.Frame(master, width=650, height=20, bd=4, relief='flat', bg='#fdeff9')
        top_frame.pack(side=tk.TOP)
        # set frame showing all buttons
        bottom_frame = tk.Frame(master, width=650, height=480, bd=4, relief='flat', bg='#fdeff9')
        bottom_frame.pack(side=tk.BOTTOM)
        # name of calculator
        my_item = tk.Label(top_frame, text="SCIENTIFIC CALCULATOR",
                           font=('Times New Roman', 18, 'bold'), fg='black', width=26, bg='#fdeff9')
        my_item.pack()
        # entry interface for inputs
        txt_display = tk.Entry(top_frame, font=('Times New Roman', 36, 'bold'), relief='solid',
                               bg='#03001e', fg='white', textvariable=self.text_input, width=60, bd=4, justify='right')
        txt_display.pack()
        ...
        # constant pi
        self.btn_pi = tk.Button(bottom_frame, **btn_params, text="pi", command=lambda: self.btn_click('pi'))
        ...
        # square root
        self.btn_sqrt = tk.Button(bottom_frame, **btn_params, text="√x", command=lambda: self.btn_click('sqrt('))
        self.btn_sqrt.grid(row=0, column=9)
        # row 1
        # changes trig function outputs to degrees
        self.btn_Deg = tk.Button(bottom_frame, **btn_params,foreground='black', activeforeground='orange', text="Deg",
        ...
        self.btn_MC.grid(row=1, column=9)
        # row 2
        ...
        self.btn_MR.grid(row=2, column=9)
        # row 3
        ...
        self.btn_M_plus.grid(row=3, column=9)
        # row 4
        # factorial function
        ...
        self.btn_0.configure(activebackground="#03001e", bg='#03001e', fg='white', width=7, bd=5)
        self.btn_0.grid(row=4, column=4, columnspan=2)
        # equals button
        self.btn_eq = tk.Button(bottom_frame, **btn_params, text="=", command=self.btn_equal)
        self.btn_eq.configure(bg='#fdeff9', activebackground='#fdeff9')
        ...
        # sinh
        self.btn_change_sign = tk.Button(bottom_frame, **btn_params, text="sinh", command=lambda: self.btn_click('sinh('))
        self.btn_change_sign.grid(row=0, column=8)
        # cosh
        self.btn_change_sign = tk.Button(bottom_frame, **btn_params, text="cosh", command=lambda: self.btn_click('cosh('))
        self.btn_change_sign.grid(row=1, column=8)
        # tanh
        self.btn_change_sign = tk.Button(bottom_frame, **btn_params, text="tanh", command=lambda: self.btn_click('tanh('))
        self.btn_change_sign.grid(row=2, column=8)
        # hypot
        self.btn_change_sign = tk.Button(bottom_frame, **btn_params, text="hyp", command=lambda: self.btn_click('hypot('))
        self.btn_change_sign.grid(row=3, column=8)
        # reciprocal
        self.btn_change_sign = tk.Button(bottom_frame, **btn_params, text=u"x-\u00B9", command=lambda: self.btn_click('1/'))
        self.btn_change_sign.grid(row=3, column=3)
        # MS
        self.btn_change_sign = tk.Button(bottom_frame, **btn_params, text="MS", command=self.memory_insert)
        self.btn_change_sign.grid(row=4, column=9)
    ...
    def memory_insert(self):
        self.recall = self.expression
